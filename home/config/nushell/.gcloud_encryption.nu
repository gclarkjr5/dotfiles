def gcp_key_encrypt [environment: string, file_to_encrypt: string, encryption_destination: string] {
      gcloud kms encrypt --project $"fxei-data-platform-($environment)" --location=europe-west3 --keyring=default-europe-west3 --key=terraform-secrets --plaintext-file=$file_to_encrypt --ciphertext-file=- | base64 > $encryption_destination
}
